...
exports.getVehicles = async (req, res) => {
  try {
    ...
    if (req.query.search) {
      ...
      filter.$or = [{ vehicleNumber: q }, { ownerName: q }, { driverName: q }, { rcNumber: q }];
    }
...
exports.getTrips = async (req, res) => {
  try {
    ...
    if (req.query.search) {
      const q = { $regex: req.query.search, $options: 'i' };
      filter.$or = [{ driverName: q }, { vehicleNumber: q }, { loadingPoint: q }, { unloadingPoint: q }, { tripNumber: q }];
    }
    ...
    res.json({
      success: true, data,
      totals: { totalTrips: agg[0]?.totalTrips || 0, totalRevenue: agg[0]?.totalRevenue || 0, totalQuantity: agg[0]?.totalQty || 0 },
