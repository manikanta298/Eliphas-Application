...
exports.getDashboardStats = async (req, res) => {
  try {
    ...
    const siteMatch = siteId ? { site: new (require('mongoose').Types.ObjectId)(siteId) } : {};
    ...
    const dieselStats = await DieselEntry.aggregate([
      { $match: dieselMatch },
      { $group: { _id: null, totalLiters: { $sum: '$presentReading' }, totalAmount: { $sum: '$totalAmount' } } },
    ]);
...
exports.getProfitReport = async (req, res) => {
  try {
    ...
    if (site) match.site = new (require('mongoose').Types.ObjectId)(site);
