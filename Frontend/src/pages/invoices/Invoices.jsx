...
export default function InvoicesPage() {
  ...
  const [invForm, setInvForm]     = useState({ site:'', invoiceDate: new Date().toISOString().split('T')[0], gstPercent:18, tdsPercent:0, notes:'', terms:'Payment due within 30 days.', dueDate:'', customer:{ name:'',company:'',address:'',gstin:'',phone:'' } });
  ...
  const fetchInvoices = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.get('/invoices', { params: { ...filters, site: filters.site || siteId, financialYear: selectedFY?.label, search: filters.search || undefined } });
  ...
  // Auto-set site when global site changes
  useEffect(() => {
    if (siteId) setFilters(f => ({ ...f, site: siteId }));
  }, [siteId]);
  ...
  return (
    <div className="animate-fade" style={{display:'flex',flexDirection:'column',gap:'var(--sp-5)'}}>
      ...
      {/* Invoice Type Tabs */}
      <div style={{ display:'flex', gap:8, borderBottom:'2px solid #e2e8f0', paddingBottom:0, flexWrap:'wrap' }}>
        {[
          { value:'customer', label:'👤 Client Invoice',  desc:'Customer billing invoices' },
          { value:'supplier', label:'🏢 Company Invoice', desc:'Supplier/vendor invoices' },
        ].map(tab => (
          <button
            key={tab.value}
            ...
            style={{
              ...
              fontSize:'0.88rem', background:'transparent', transition:'all 0.15s',
              borderBottom: filters.type === tab.value ? '3px solid #1a3c8f' : '3px solid transparent',
              color: filters.type === tab.value ? '#1a3c8f' : '#64748b',
              marginBottom:'-2px',
            }}
          >
            {tab.label}
        ...
        <div style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:6, paddingBottom:8 }}>
          <span style={{ fontSize:'0.75rem', color:'#64748b' }}>
            {filters.type === 'customer' ? '👤 Showing Client Invoices' : '🏢 Showing Company Invoices'}
          </span>
        </div>
      </div>
      ...
      <div className="filters-bar">
        <input className="form-control" style={{minWidth:200,flex:1}} placeholder="🔍 Search invoice #, company…"
        ...
        <button className="btn btn-outline" onClick={() => setFilters({ type: filters.type, site: siteId, paymentStatus:'', startDate:'', endDate:'', search:'' })}>✕ Reset</button>
      </div>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:'var(--sp-5)'}}>
          ...
          <div>
            ...
            <div className="form-grid">
              <div className="form-group"><label className="form-label">Invoice Date * <span style={{color:'var(--clr-danger)',fontSize:'0.75rem'}}>Required</span></label>
                <input type="date" className="form-control" value={invForm.invoiceDate} onChange={e=>setInvForm({...invForm,invoiceDate:e.target.value})} required style={{border: !invForm.invoiceDate ? '1.5px solid var(--clr-danger)' : undefined}} />
              </div>
