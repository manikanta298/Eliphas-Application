...
export default function PurchasePage() {
  ...
  useEffect(() => { if (selectedFY) fetchPurchases(); }, [selectedFY, filter, siteId]);
  ...
  return (
    <div className="page animate-fade">
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '14px', flexWrap: 'wrap' }}>
        <input className="form-control" placeholder="🔍 Search supplier, invoice no..." value={search} onChange={e => setSearch(e.target.value)} style={{ maxWidth: 280 }} />
        ...
        {(search || filter.paymentStatus) && <button className="btn btn-secondary btn-sm" onClick={() => { setSearch(''); setFilter(f => ({ ...f, paymentStatus: '' })); }}>✕ Reset</button>}
      </div>
      ...
      {loading ? <div className="loading-center"><div className="spinner" /></div> :
        purchases.filter(p =>
          ...
          <div className="empty-state">
            ...
            <p>{search ? `No results for "${search}"` : `No purchase entries yet for ${selectedFY?.label}. Add first purchase.`}</p>
          </div>
        ) : (
          ...
          <div className="modal" style={{ maxWidth: '780px', width: '95%', maxHeight: '92vh', overflow: 'auto' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', marginBottom: '16px' }}>
              <ComboInput
                ...
                options={suppliers}
                onSelect={s => setForm(f => ({ ...f, supplier: s._id, supplierName: s.name }))}
                onChange={name => setForm(f => ({ ...f, supplierName: name, supplier: '' }))}
