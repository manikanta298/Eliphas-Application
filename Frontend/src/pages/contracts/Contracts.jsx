  ...
  return (
    <div className="animate-fade" style={{ display:'flex', flexDirection:'column', gap:'var(--sp-5)' }}>
      ...
      <div className="filters-bar">
        <input className="form-control" placeholder="🔍 Search by title, site, billing type..." value={search} onChange={e => setSearch(e.target.value)} style={{maxWidth:320}} />
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={10} style={{textAlign:'center',padding:'40px'}}><div className="spinner" style={{margin:'auto'}}/></td></tr>
                ...
                c.site?.name?.toLowerCase().includes(search.toLowerCase()) ||
                c.billingType?.toLowerCase().includes(search.toLowerCase()) ||
                c.contractNumber?.toLowerCase().includes(search.toLowerCase())
              ).length === 0 ? (
              <tr><td colSpan={10}><div className="empty-state"><div className="empty-state-icon">📋</div><p>{search ? `No results for "${search}"` : 'No contracts yet'}</p></div></td></tr>
            ) : contracts.filter(c =>
                !search ||
                c.title?.toLowerCase().includes(search.toLowerCase()) ||
                c.site?.name?.toLowerCase().includes(search.toLowerCase()) ||
